...
const SeriesTrailers = ({ trailers }: Props) => {
	...
	return (
		<div className="flex w-[680px] flex-col">
			<div className="flex h-8 w-full items-end">
				{trailers.slice(0, 5).map((trailer, index) => (
					<button
						key={index}
						className={cn(
							'w-full min-w-32 cursor-pointer rounded-t-md bg-gray-600/40 px-4 text-white backdrop-blur-md duration-150 ease-in-out',
