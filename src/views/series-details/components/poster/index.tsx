...
const Poster = ({ posterPath, trailer, status }: Props) => {
	...
	return (
		<div className="relative h-full w-[336px] shrink-0 overflow-hidden rounded-3xl">
			{posterPath && (
				<img src={posterPath} className="size-full object-cover" />
			)}

			<div className="border-px absolute top-5 left-5 flex h-10 cursor-default items-center justify-center rounded-full border border-white/10 bg-gradient-to-t from-black/70 to-black/30 px-3.5 backdrop-blur-sm transition-all duration-300 ease-in-out">
				<TextFade className="text-[13px] leading-4 text-white">
					{status}
				</TextFade>
			</div>

			<button className="border-px absolute top-5 right-5 flex size-10 cursor-pointer items-center justify-center rounded-full border border-white/10 bg-gradient-to-t from-black/70 to-black/30 backdrop-blur-sm transition-all duration-300 ease-in-out hover:scale-110 hover:bg-black/50 hover:shadow-[0_0_20px_rgba(255,255,255,0.1)] hover:backdrop-blur-md">
				<FaRegStar className="size-4" />
			</button>

			<button
				className="group absolute bottom-0 left-0 h-1/3 w-full cursor-pointer"
